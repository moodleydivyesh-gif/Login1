...
ArrayList<Messages> storedList = Messages.getStoredList();                      // a. Display sender and recipient of all stored messages
if (storedList.isEmpty()) {                                                     // Reads from storedList object list using getStoredList()
...
case "b":
ArrayList<Messages> stored = Messages.getStoredList();              // b. Display the longest stored message
if (stored.isEmpty()) {                                            // Loops storedList, compares getMessageText().length() directly
System.out.println("No stored messages.");
} else {
...
case "c":
System.out.print("Enter message ID or recipient number to search: ");                    // c. Search for a message by recipient number
 String searchID = input.nextLine().trim();                                              // Loops all 3 lists, matches getRecipient().equals()
...
if (msg.getRecipient().equals(searchID)) {
System.out.println("ID       : " + msg.getMessageID());
System.out.println("Hash     : " + msg.getMessageHash());
System.out.println("Recipient: " + msg.getRecipient());
System.out.println("Message  : " + msg.getMessageText());
...
if (!foundC) {
System.out.println("No message found for: " + searchID);
}
break;

case "d":
 System.out.print("Enter recipient number to search: ");                                // d. Search all messages for a particular recipient
String searchRecipient = input.nextLine().trim();                                       // Loops sentList + storedList + disregardedList
...
ArrayList<Messages> allF = new ArrayList<>();      // f. Display full report of all message
allF.addAll(Messages.getSentList());               // Loops all 3 lists — shows Message Hash, Recipient, Message
allF.addAll(Messages.getStoredList());
allF.addAll(Messages.getDisregardedList());
if (allF.isEmpty()) {
...
System.out.printf("%-25s %-20s %s%n",msg.getMessageHash(),msg.getRecipient(),msg.getMessageText());
}
}
break;

case "x":
inMenu = false; // go back to main menu
